...
const data: Info[] = [
    {
        info: RatanTataInformation,
        reference: "Ratan Tata",
        relevance: 0.9
    },
    {
        info: NMurthyInformation,
        reference: "Narayan Murthy",
        relevance: 0.8
    }
];

const pcIndex = pc.index<Info>('ai-project-index');
...
async function storeEmbeddings() {
    await Promise.all(
        data.map(async (item, index) => {
            const embedding = await generateEmbeddings(item.info);
            const record:PineconeRecord<Info> = {
                id: `INFO-${index}`,
                values: embedding,
                metadata: item
            }
            await pcIndex.upsert([record]);
        })
    );
}

async function queryEmbeddings(input: string) {
...
async function askQuestion(contextInfo: string, userInput:string) {
    context.push({
        role: "assistant",
        content: "Answer the next question based on the given context: "+contextInfo
    });
    context.push({
        role: "user",
        content: userInput
...
process.stdin.addListener('data', async function(input){
    ...
    const queryResultResponse = await queryEmbeddings(userInput);
    askQuestion(queryResultResponse.metadata?.info!, userInput);
})
